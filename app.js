...
bot.on("message", async (message) => {
    ...
    let keys = Object.keys(transes);

    if (keys.includes(message.chat.id) || keys.includes(message.chat.id.toString())){
        let ch = transes[message.chat.id.toString()];
        if (ch['users'].includes(message.from.id.toString())){
            for (let user of ch['users']){
                if (ch[user]['is_capt']){
                    await bot.sendMessage(
                        message.chat.id,
                        "[ 🚧 ] - در حال پردازش ..."
                        ...
                        await network.getAccountInfo(message.text, async (trns) => {
                            if (trns.address === undefined && trns.activated === undefined && trns.date_created === undefined){
                                ...
                                await bot.editMessageText("[ 🚧 ] - هش شما ست شد", {
                                    chat_id: ch[message.from.id]['chat_id'],
                                    message_id: ch[message.from.id]['message_id']
                                })
                                ch[user]['in_capt'] = false;
                                if (ch[user]['mask'] == "owner"){
                                    ch['text'] = ch['text'].replace("[ 🎾 ] - از: ...", `[ 🎾 ] - از: <code>${message.text}</code>`);
                                } else if (ch[user]['mask'] == "member"){
                                    ch['text'] = ch['text'].replace("[ 🍬 ] - به: ...", `[ 🍬 ] - به: <code>${message.text}</code>`); //[ 🍬 ] - به: ...
                                }
                                await bot.editMessageText(ch['text'], {
                                    chat_id: message.chat.id,
                                    ...
                                    reply_markup: {
                                        inline_keyboard: [
                                            [
                                                {
                                                    text: "تایید ✅",
                                                    callback_data: `accept_${message.chat.id}`
                                                }
                                            ]
                                        ]
                                    }
...
bot.on("callback_query", async (call) => {
    if (call.data.startsWith("close")){
        ...
        delete transes[call.message.chat.id][uid]['partner'];
        transes[call.message.chat.id][uid]['partner'] = call.from.id;
        transes[call.message.chat.id][uid]['in_capt'] = true;
        transes[call.message.chat.id]['text'] = txt;
        transes[call.message.chat.id]['users'].push(call.from.id);
        transes[call.message.chat.id][call.from.id] = {
            mask: "member",
            partner: uid,
            in_capt: true
        }

        transes[call.message.chat.id]['timestamp'] = date.getTime() + 300000;
